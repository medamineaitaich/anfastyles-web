...
const BENEFITS = [
  {
    ...
    description: 'Easy to wear for garden mornings, errands, and laid-back weekends.',
    icon: Leaf,
  },
  {
    title: 'Gardening-inspired statement design',
    description: 'A quiet conversation starter for people who care about soil, compost, and renewal.',
    icon: Sprout,
  },
  {
    title: 'Made to order to reduce waste',
    description: 'Produced after purchase to help avoid unnecessary inventory and overproduction.',
    icon: Recycle,
  },
  {
    title: 'Perfect for composters and growers',
    description: 'Designed for eco-minded people who like their clothes to reflect what they believe.',
    icon: CheckCircle2,
  },
];
const FAQS = [
  {
    question: 'Is this shirt made to order?',
    answer: 'Yes. It is produced after your order is placed to help reduce waste.',
  },
  {
  ...
  },
  {
    ...
    answer: 'Use the size guide and select your preferred size before checkout.',
  },
];
const SHOWCASE_IMAGES = [
  ...
  },
  {
    key: 'natural',
    label: 'Natural',
    description: 'A warm cream tone that brings out the roots-and-leaves artwork beautifully.',
  ...
  },
  {
    key: 'light-pink',
    label: 'Light Pink',
    description: 'A gentle pastel option with a playful gardening feel.',
    src: '/offers/compost/compost-pink.jpg',
  },
  {
    key: 'ice-grey',
    label: 'Ice Grey',
  ...
  useEffect(() => {
    ...
    const loadProduct = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error('Unable to load the Compost T-Shirt offer right now.');
        }
      ...
      <main className="overflow-x-hidden bg-[radial-gradient(circle_at_top,_rgba(160,198,150,0.18),_transparent_40%),linear-gradient(180deg,_rgba(250,247,240,0.98)_0%,_rgba(247,243,233,0.92)_48%,_rgba(255,255,255,1)_100%)] pb-16">
            ...
            <div className="order-2 space-y-6 lg:order-1">
              ...
              <div className="flex flex-wrap items-end gap-5">
                ...
                <div className="space-y-1 text-sm text-muted-foreground">
                  <p>Sizes and colors stay synced with the real product variations.</p>
                  <p>Checkout completes on this page using the existing secure flow.</p>
                </div>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                {TRUST_BADGES.map((badge) => (
                  <div key={badge} className="rounded-2xl border border-border/60 bg-background/75 px-4 py-3 text-sm font-semibold shadow-sm">
                    {badge}
                  </div>
                ))}
              </div>
            ...
            <div className="order-1 lg:order-2">
              <div className="rounded-[2rem] border border-border/60 bg-card/90 p-4 shadow-[0_20px_60px_-28px_rgba(86,102,66,0.4)] backdrop-blur">
                {loading ? (
                  ...
                  <img
                    src={heroImage}
                    alt={product?.name || 'Compost T-Shirt'}
        ...
        <section className="container-custom py-10 md:py-14">
          <div className="mb-8 max-w-2xl">
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-primary/80">Why people love it</p>
            <h2 className="mt-2 text-3xl font-bold text-balance md:text-4xl">A focused offer with an eco-conscious feel</h2>
          </div>
...
export default CompostOfferPage;
